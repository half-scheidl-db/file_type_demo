...
function toRecord(row: Record<string, string | null>): InspectionRecord {
  return {
    ...
    capStatus: (row.cap_status ?? "UNKNOWN") as InspectionRecord["capStatus"],
    labelAlignment: (row.label_alignment ?? "UNKNOWN") as InspectionRecord["labelAlignment"],
    labelAttachment: (row.label_attachment ?? "UNKNOWN") as InspectionRecord["labelAttachment"],
    housingCondition: (row.housing_condition ?? "UNKNOWN") as InspectionRecord["housingCondition"],
    contamination: (row.contamination ?? "UNKNOWN") as InspectionRecord["contamination"],
    imageObstruction: row.image_obstruction === "true",
    imageQuality: (row.image_quality ?? "INSUFFICIENT") as InspectionRecord["imageQuality"],
...
const projection = `
  ...
  housing_condition,
  contamination,
  CAST(image_obstruction AS STRING) AS image_obstruction,
  image_quality,
  status,
  observed_issue,
